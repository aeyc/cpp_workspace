...
bool TcpServer::acceptConn() 
{
  ...
  // failed), it prints to std::cout and error and returns false.
    if (sock_fd<0){
        std::cout << "ERROR: FILE DESCRIPTOR IS NOT VALID" << std::endl;
        return false;
    }
  ...
  // method inet_ntoa. Then it increments the connection counter and returns true.
    ...
    std::cout << "Ip of client" <<ip_client<< std::endl;
    connections_count +=1;
    return true;
}
...
int TcpServer::receive(std::shared_ptr<std::array<char,MTU>> buf)  
{
  ...
  // descriptor to -1 and returns -1.
    ...
    int rcv_size = int(recv(sock_fd,buf->data(),buf->size(),0));
    if (rcv_size < 0 ){
        std::cout << "ERROR: RECV OPERATION FAILED" << std::endl;
        close(sock_fd);
        sock_fd = -1;
        return -1;
    }
  ...
  size_t size_to_tx) 
{
  ...
  // SUGGESTION2: access the underlying C-array of buffer with buf->data().
    int sent_bytes = int(send(sock_fd, buf->data(), size_to_tx, 0));
    if (sent_bytes < 0 ){
        std::cout << "ERROR: TRANSMIT OPERATION FAILED" << std::endl;
        close(sock_fd);
        sock_fd = -1;
        return -1;
    }
